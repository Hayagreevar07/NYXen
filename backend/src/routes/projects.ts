...
router.get('/', (req: Request, res: Response) => {
  try {
    ...
    if (status && typeof status === 'string') {
      projects = ProjectStore.getByStatus(status as any);
    } else {
    ...
    const enriched = projects.map((p) => {
      const summary = MeasurementStore.getProjectSummary(p.id);
      return {
        ...p,
...
router.get('/:id', (req: Request, res: Response) => {
  try {
    ...
    const summary = MeasurementStore.getProjectSummary(project.id);
    const measurements = MeasurementStore.getByProjectId(project.id);
